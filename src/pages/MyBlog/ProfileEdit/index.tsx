...
const ProfileEditBox = (userData: ILoginUserData) => {
  const memberInfo = userData.memberInfo;
  const memberUrls = memberInfo.memberPortfolioUrls;
  const userEditFormik = useFormik({
    initialValues: {
      ...
      gitEmail: memberInfo.gitEmail,
      birthday: memberInfo.birthday,
      introduce: memberInfo.introduce,
      hashTag: memberInfo.hashTag,
      memberInfoId: memberInfo.memberInfoId,
      phoneNumber: memberInfo.phoneNumber,
      major: memberInfo.major,
      positionType: memberInfo.positionType,
      githubUrl: memberUrls[0].webUrl,
      blogUrl: memberUrls[1].webUrl,
      resumeUrl: memberUrls[2].webUrl,
    },
  ...
  const onSubmit = async (values: IUserEditDataType) => {
    ...
    try {
      await API.updateUserData(memberData);
    } catch (err) {
      // await navigate(-1);
      // await openModal
      console.log(err);
            ...
            <FormInner onSubmit={userEditFormik.handleSubmit}>
              ...
              <FormButtonWrapper>
                <GDSCButton
                  ...
                  color={'googleBlue'}
                  onClick={() => {
                    onSubmit(userEditFormik.values);
                  }}
                />
              </FormButtonWrapper>
            </FormInner>
